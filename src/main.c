...
void read_value(char *pReadBuffer, uint32_t *pReadValue)
{
  ...
  if (typeExpected == NUMBER_STRING)
  {
    ...
    if (pReadBuffer[0] == 0x71 && pReadBuffer[5] == 0xFF && pReadBuffer[6] == 0xFF && pReadBuffer[7] == 0xFF)
    { // this is a complete number return
...
void button_press_detect()
{
  ...
  if (button1_page == 0x00 && button1_ID == 0x01 && button1_release == 0x00)
  {                                            // checking on which button has been pressed; it is true if button1 on page0 would be released
